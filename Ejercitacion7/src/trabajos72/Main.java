...
public class Main {
    public static void main(String[] args) {
        ...
        Libro[] estanteria = new Libro[datos.length];
        ...
        for (int i = 0; i < datos.length; i++) {
            ...
            estanteria[i] = new Libro(titulo, isbn, paginas);
        }

        Biblioteca miBiblioteca = new Biblioteca();

        for (int i = 0; i < estanteria.length; i++) {
            miBiblioteca.agregarLibro(estanteria[i]);
        }

        System.out.println("--- Catálogo Completo de la Biblioteca ---");
        miBiblioteca.mostrarCatalogo();

        System.out.println("\nCantidad total de libros en la biblioteca: " + miBiblioteca.contarTotalLibros());

        System.out.println("El libro en el índice (5) es '" + estanteria[5].getTitulo() + 
                           "' y tiene " + estanteria[5].getCantidadPaginas() + " páginas.");
    }
}