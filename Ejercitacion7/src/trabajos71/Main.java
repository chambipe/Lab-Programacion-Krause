...
public class Main {
    public static void main(String[] args) {
        ...
        Producto[] loteProductos = new Producto[datos.length];
        ...
        for (int i = 0; i < datos.length; i++) {
            ...
            loteProductos[i] = new Producto(nombre, codigo, precio);
        }

        Inventario miInventario = new Inventario();

        for (int i = 0; i < loteProductos.length; i++) {
            miInventario.agregarProducto(loteProductos[i]);
        }

        System.out.println("--- Listado de Productos en Inventario ---");
        miInventario.mostrarInventario();
        
        System.out.println("\nTotal de artículos registrados: " + miInventario.obtenerTotalProductos());
    }
}